...
const highlightedModelId = ref<string | undefined>(props.selectedModelId || undefined)

watch(() => props.selectedModelId, (modelId) => {
  highlightedModelId.value = modelId || undefined
}, { immediate: true })

const live2dDialog = useFileDialog({ accept: '.zip', multiple: false, reset: true })
live2dDialog.onChange(async (files) => {
  if (files === null || files.length === 0)
    return
  if (!files[0].name.endsWith('.zip'))
    return
  await live2dStore.uploadModel(files[0])
})
...
function handleMobilePick() {
  emit('pick', highlightedModelId.value)
  emit('close')
}

async function handleRename(modelId: string, name: string) {
  await live2dStore.renameModel(modelId, name)
}
</script>
        ...
        >
          ...
          >
            <img
              ...
              :class="[highlightedModelId && highlightedModelId === model.id ? 'ring-3 ring-primary-400' : 'ring-0 ring-transparent']"
              transition="all duration-200 ease-in-out"
            >
            <div
              v-else
              bg="neutral-100 dark:neutral-900"
              relative h-full w-full flex flex-col items-center justify-center gap-2 overflow-hidden rounded-xl
              :class="[highlightedModelId && highlightedModelId === model.id ? 'ring-3 ring-primary-400' : 'ring-0 ring-transparent']"
              transition="all duration-200 ease-in-out"
            >
