...
import ModelSettings from '@/components/live2d/ModelSettings.vue'
...
async function extractColorsFromModel() {
  ...
  try {
    ...
    palette.value = Object.values(paletteFromVibrant).map(color => color?.hex).filter((it): it is string => typeof it === 'string')
  }
  finally {
